...
public class HeapSortCraft 
{
	...
	public static void heapSortIncrease(int[] arr)
	{
		...
		//下一次只需要将number减一即可，因为最后一个数字已经是最大了
		number = number - 1;

		//第二次对数组全部元素遍历找出非叶子节点，并将树调整为大顶堆的格式
		for (int i = number - 1; i >= 0; i--)
		{
			//对数组全部元素遍历找出非叶子节点，并将树调整为大顶堆的格式
			//非叶子节点说明该节点的左右子节点都在数组范围内
			if ((2 * i + 1) < arr.length && (2 * i + 2) < arr.length)
			{
				//1.两边元素都还有待排序， 
	...
	public static void main(String[] args) 
	{
		...
		//int[] array = HeapSortCraft.getArray(ARRSIZE, MAXNUM);
		...
		HeapSortCraft.heapSortIncrease(array);
